...
class PbsHealthSpider(scrapy.Spider):
    name = "pbs_health"
    allowed_domains = ["pbs.org"]
    start_urls = ["https://www.pbs.org/newshour/health"]
    article_count = 0
    max_articles = 600

    custom_settings = {
        'FEED_FORMAT': 'csv',
        'FEED_URI': 'pbs_health_articles.csv',
    ...
    def parse(self, response):
        ...
        keywords = ['health', 'mental', 'medical', 'covid', 'pandemic', 'cancer', 'hospital',
                    'disease', 'vaccine', 'wellness', 'nutrition', 'fitness', 'epidemic']
