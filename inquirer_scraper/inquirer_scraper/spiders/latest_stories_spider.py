...
class LatestStoriesSpider(scrapy.Spider):
    ...
    categories = {
        ...
        'politics': ('https://www.inquirer.net/category/philippine-elections/page/{}/', 2000)
    }
    ...
    health_keywords = [
        ...
        'virus', 'infection', 'surgery', 'illness'
    ]

    def start_requests(self):
        for category, info in self.categories.items():
            if isinstance(info, list):  # for health
                ...
                url_pattern, _ = info
                for i in range(1, 1001):  # up to 1000 pages for others
                    url = url_pattern.format(i)
                    yield scrapy.Request(url=url, callback=self.parse, meta={'category': category})
