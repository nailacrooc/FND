...
TFIDF_FILE = "politics_tfidf.pkl"
LDA_FILE = "politics_lda.pkl"
SVM_FILE = "politics_svm.pkl"
...
def train_politics_model():
    print("Training new politics model...")

    df = pd.read_csv("C:/Users/johnj/ScrapyTest/ScrapyTest/FND-LATEST/FND/POLITICS-cleaned.csv", encoding="ISO-8859-1", on_bad_lines="skip")
    ...
    print(f"\n[Politics] Accuracy: {accuracy_score(y_test, y_pred):.4f}")
    print("[Politics] Classification Report:")
    print(classification_report(y_test, y_pred, target_names=['Not Credible', 'Credible']))

def load_politics_models():
    # If any pickle file missing, train first
    if not (os.path.exists(TFIDF_FILE) and os.path.exists(LDA_FILE) and os.path.exists(SVM_FILE)):
        train_politics_model()
...
def predict_politics_label(headline, content):
    tfidf, lda, svm = load_politics_models()
